...
def create_query(user_query, click_prior_query, filters, sort="_score", name_option="name", sortDir="desc", size=10, source=None):
    query_obj = {
        'size': size,
        "sort": [
            {sort: {"order": sortDir}}
...
def search(client, user_query, index="bbuy_products", name_option="name", sort="_score", sortDir="desc"):
...
if __name__ == "__main__":
    ...
    general.add_argument("--synonyms", type=bool, default=False,  help="If true the match is made on Synoymns")
    ...
    host = args.host
    port = args.port
    name_option = "name.synonym" if args.synonyms else "name"
    if args.user:
        password = getpass()
