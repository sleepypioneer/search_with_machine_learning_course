...
def search(client, user_query, index="bbuy_products", sort=None, sortDir="desc", is_boost=False, category_threshold=0):
    ...
    for i in range(0, len(categories)):
        if probs[i] > category_threshold:
            current_cat = categories[i].replace("__label__", "")
            category_list.append(current_cat)
        else:
            break

    query_obj = create_query(user_query, click_prior_query=None, filters=None, sort=sort, sortDir=sortDir, source=["name", "shortDescription"], categories=category_list, is_boost=is_boost)
...
if __name__ == "__main__":
    ...
    general.add_argument("-c", '--cat_threshold', type=float, default=0.3,
